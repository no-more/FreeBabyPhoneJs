...
export class ReconnectService implements OnDestroy {
	...
	/** Number of auto-reconnect attempts made so far (0–5). */
	readonly attempts = signal(0);
	/** Timestamp (ms) of the last disconnect event, or null if never disconnected this session. */
	readonly lastDisconnectAt = signal<number | null>(null);
	...
	attach(peer: RTCPeerConnection): void {
		...
		this.pc = peer;
		this.attempts.set(0);
		this.lastDisconnectAt.set(null);
	...
	private handleStateChange(): void {
		...
		if (state === 'connected') {
			...
			this.lastDisconnectAt.set(Date.now());
